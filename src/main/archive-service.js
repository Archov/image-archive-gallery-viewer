...
class ArchiveService {
    ...
    async initializeArchivesDb() {
        try {
            ...
            await fs.writeFile(this.archivesDbPath, JSON.stringify({
                archives: {},
            ...
            try {
                ...
                files.forEach(file => {
                    if (!file.fileHeader.flags.directory &&
                        this.isImageFile(file.fileHeader.name)) {
    ...
    async extract7z(archivePath, extractPath, progressCallback) {
        return new Promise((resolve, reject) => {
            ...
            stream.on('end', () => {
                ...
                this.scanDirectoryForImages(extractPath).then(files => {
                    resolve(files);
                }).catch(reject);
            });
    ...
    async processArchive(archivePath, progressCallback, forceReprocess = false) {
        ...
        console.log(`[ARCHIVE] Archive type: ${metadata.type}, size: ${(metadata.size / 1024 / 1024).toFixed(2)}MB`);

        // Create extraction directory
        const extractDir = path.join(this.tempDir, `extract_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`);
